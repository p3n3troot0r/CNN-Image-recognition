# THIS CODE IS DEPRECIATED, USE:
# patches_artifact_reduction_cnn.py
...
# model = Sequential()
# model.add(Convolution2D(nb_filter=128, nb_row=9, nb_col=9, border_mode='same', init='glorot_normal', input_shape=(1, 256, 256)))
# model.add(BatchNormalization())
# model.add(Activation('relu'))
# model.add(MaxPooling2D((2, 2), border_mode='same'))
# model.add(Dropout(0.5))
# model.add(Convolution2D(nb_filter=64, nb_row=1, nb_col=1, border_mode='same', init='glorot_normal'))
# model.add(BatchNormalization())
# model.add(Activation('relu'))
# model.add(UpSampling2D((2, 2)))
# model.add(Dropout(0.5))
# model.add(Convolution2D(nb_filter=1, nb_row=5, nb_col=5, border_mode='same', init='glorot_normal'))
# model.add(BatchNormalization())
...
# model.load_weights(filepath="weights.104-1.03.hdf5")
...
for i in range(4000):
    ...
    for j in range(load_batch_size):
        residual_data_grid = blockshaped(residual_data[j], 28, 28)
        compressed_data_grid = blockshaped(compressed_data[j], 28, 28)
    ...
    # model.fit(compressed_data_norm, residual_data,
    #             nb_epoch=10,
    #             batch_size=1,
    #             shuffle=True,
